 ...
 *
 * The KeycloakSteve authentication strategy authenticates requests by delegating to
 * Keycloak using the OAuth 2.0 protocol.
 *
 ...
 *
 * Options:
 *   - `clientID`      your Keycloak application's Client ID
 *   - `clientSecret`  your Keycloak application's Client Secret
 *   - `callbackURL`   URL to which Keycloak will redirect the user after granting authorization
 ...
 *         callbackURL: 'https://www.example.net/auth/keycloak..../callback',
 *         userAgent: 'myapp.com'
 *       },
...
function Strategy(options, verify) {
  options = options || {};
  options.authorizationURL = options.authorizationURL || 'https://there is no default';
  options.tokenURL = options.tokenURL || 'https://supply your tokenURL with this suffix /login/oauth/access_token';
  options.scopeSeparator = options.scopeSeparator || ',';
  options.customHeaders = options.customHeaders || {};

  if (!options.customHeaders['User-Agent']) {
    options.customHeaders['User-Agent'] = options.userAgent || 'passport-keycloaksteve';
  }

  OAuth2Strategy.call(this, options, verify);
  this.name = 'keycloaksteve';
  this._userProfileURL = options.userProfileURL || 'https://there is no default api.github.com/user';
...
Strategy.prototype.userProfile = function(accessToken, done) {
  ...
  this._oauth2.get(this._userProfileURL, accessToken, function (err, body, res) {
    ...
    profile.provider  = 'keycloaksteve';
    profile._raw = body;
    profile._json = json;
